...
public class SingleSenderCreator implements RootCreator<SingleSender>{
  ...
  public static class SingleSenderRootExec extends BaseRootExec<SingleSenderRootExec.SingleSenderIterationState> {
    ...
    SingleSenderRootExec(FragmentContext context, RecordBatch batch, SingleSender config) throws OutOfMemoryException {
      ...
      tunnel = DrillbitDelegatingAccountingDataTunnel.of(context.getDataTunnel(endpoint.getEndpoint()), getSendAvailabilityNotifier());
    ...
    public IterationResult innerNext() {
      ...
      final SingleSenderIterationState state = restorePendingState();
      if (done) {
        incoming.kill(true);
